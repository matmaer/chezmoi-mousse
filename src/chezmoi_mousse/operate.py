...
from chezmoi_mousse.commands import run, CommandIO
...
# class InspectTabs(Widget):

#     # needs to end up in an animated slidebar
...
#                 yield VerticalScroll(Pretty(run("chezmoi_config")))
#                 yield VerticalScroll(Pretty(run("chezmoi_data")))
...
class ManagedFiles(DirectoryTree):
    ...
    managed = CommandIO().managed_paths
...
class OperationScreen(Screen):
    ...
    # set layers for operationtabs and inspecttabs
    # DEFAULT_CSS = {
    #     "layers": {"operate", "inspect", "slidebar"}
    # }
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical():
            with TabbedContent(
                ...
                yield ChezmoiStatus()
                yield VerticalScroll(ManagedFiles("chezmoi_data"))
        yield Footer()
